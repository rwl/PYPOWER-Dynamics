...
from pypower.idx_bus import REF, PV, PQ
...
def bustypes(bus, gen):
    ...
    Cg = sparse((array(gen.status) > 0, (gen.bus, range(ng))), (nb, ng))
    # number of generators at each bus that are ON
    bus_gen_status = (Cg * ones(ng, int)).astype(bool)

    # form index lists for slack, PV, and PQ buses
    ref = find((array(bus.bus_type) == REF) & bus_gen_status) # ref bus index
    pv  = find((array(bus.bus_type) == PV)  & bus_gen_status) # PV bus indices
    pq  = find((array(bus.bus_type) == PQ) | ~bus_gen_status) # PQ bus indices
